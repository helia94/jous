...
# API Endpoint
API_URL = "https://jous.app/api/blog"

# Function to send POST request with retries
def send_request(session, payload, retries=2):
    for attempt in range(retries):
        try:
            response = session.post(API_URL, json=payload, timeout=90)  # 60 seconds timeout per request
            response.raise_for_status()  # Raise exception for 4xx/5xx errors
            return response.text  # Successfully sent request
...
# Make HTTP requests using a persistent connection
with httpx.Client(headers={"Content-Type": "application/json"}) as session:
    for post in blog_posts:
        payload = {"url": post["URL"], "title": post["title"]}
        response = send_request(session, payload)
        if response:
